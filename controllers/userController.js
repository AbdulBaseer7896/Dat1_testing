...
exports.login = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      datAccount: getDataSession,
      allowedDomains: relatedDomains
    });
  } catch (error) {
    console.error('Login error:', error.message);
    res.status(500).json({ error: 'Server error during login' });
...
exports.isLoggedIn = async (req, res) => {
  ...
  try {
    ...
    const existingSession = await UserSession.findOne({ userId: decoded.userId, token, isActive: true });
    if (!existingSession) {
    ...
    const getDataSession = getPermission.dataSessionId
      ? await DataSession.findById(getPermission.dataSessionId)
      : null;
    const relatedDomains = await WhitelistedDomain.find({
      _id: { $in: getPermission.domains || [] }
    });
